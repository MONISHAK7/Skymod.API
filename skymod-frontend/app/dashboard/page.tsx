...
const Dashboard = () => {
  ...
  const statusCounts = properties.reduce((acc: any, curr: any) => {
  ...
  <div className="p-6 min-h-screen bg-gradient-to-tr from-[#f0f4f8] via-[#e8edf4] to-[#f9fbfd] text-gray-900">
    ...
    <div className="flex items-center justify-between mb-8">
      <div>
        ...
        {user && (
          <p className="text-gray-700 mt-1 text-md">Welcome back, <span className="font-medium text-blue-800">{user.name}</span></p>
        )}
      </div>
      <button
    ...
    <div className="grid md:grid-cols-2 gap-6 mb-10">
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6">
        <h2 className="text-lg font-semibold mb-4 text-blue-800">📊 Purchase vs Selling Prices</h2>
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6">
        <h2 className="text-lg font-semibold mb-4 text-blue-800">📈 Status Distribution</h2>
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
    ...
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
      {properties.map((prop: any) => (
        ...
        >
          ...
          {/* Image */}
          <img
            src={prop.imageUrl || '/realestate.jpg'} // get the image url from db or use fallback image
            alt="Property"
            className="w-full h-48 object-cover"
          />
          <h3 className="text-xl font-bold text-blue-800 mb-2">{prop.address}</h3>
          <ul className="text-sm text-gray-700 space-y-1">
            <li><strong>📅 Purchase:</strong> {prop.purchaseDate}</li>
            <li><strong>💰 Bought:</strong> ${prop.purchasePrice}</li>
            <li><strong>🏷️ Sold:</strong> ${prop.sellingPrice}</li>
            <li>
              <strong>Status:</strong>{' '}
              <span className={`font-semibold ${prop.status === 'Sold'
                ? 'text-green-600'
                : prop.status === 'Ongoing'
                ? 'text-yellow-600'
                : 'text-blue-600'
